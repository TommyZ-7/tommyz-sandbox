...
import { EffectType, EffectParticle } from "./effects";
...
export default function App() {
  ...
  // エフェクト選択用のState
  const [selectedEffect, setSelectedEffect] = useState<EffectType>("sparkle");
  ...
  const predictWebcam = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(
        `手の検出中にエラーが発生しました: ${error instanceof Error ? error.message : String(error)
        }`
      );
    }
              ...
              onUserMediaError={(error) => {
                ...
                setErrorMessage(
                  typeof error === "object" &&
                    ...
                    "name" in error &&
                    "message" in error
                    ? `Webcamエラー: ${(error as DOMException).name} - ${(error as DOMException).message
                    }`
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="flex items-center space-x-2 text-sm">
              ...
              >
                <option value="sparkle">キラキラ (Sparkle)</option>
                <option value="ripple">波紋 (Ripple)</option>
